...
class SnapMMDUnifiedPartiallyUnlabeled(Dataset):
    ...
    DATASET_CONFIGS = {
        'GoM': {
            ...
            'data_name_unlabeled': 'GoM_data_interp_val.npz',
            'data_shape': [2],
            'has_x_scaling': False
        },
        'LV': {
            ...
            'data_name_unlabeled': 'LV_data_interp_val.npz',
            'data_shape': [2],
            'has_x_scaling': False
        },
        'PBMC': {
            ...
            'data_name_unlabeled': 'processed_pbmc_data_sub500_every_2_until20_interp_val.npz',
            'data_shape': [30],
            'has_x_scaling': True
        },
        'Repressilator': {
            ...
            'data_name_unlabeled': 'Repressilator_data_interp_val.npz',
            'data_shape': [3],
            'has_x_scaling': False
        }
    }
    ...
    def __init__(
        ...
        data_path_unlabeled = os.path.join(base_dir, self.config['data_dir'], self.config['data_name_unlabeled'])
        self.full_dataset_unlabeled = np.load(data_path_unlabeled)
        
        # NOTE: indexing [:-1] is to remove the last time point, which is the target for forecasting benchmarks.
        self.data_labeled = self.full_dataset['Xs'][:-1]
        self.data_unlabeled = self.full_dataset_unlabeled['Xs'][:]
        if use_labeled_data_only:
            self.data = self.data_labeled
        else:           
            self.data = np.concatenate([self.data_labeled, self.data_unlabeled], axis=0)
    ...
    def __getitem__(self, idx):
        if self.testing_method == "forecast":
            ...
            if source_idx < self.data_labeled.shape[0] and target_idx < self.data_labeled.shape[0]:
                dt_value = self.d_fun(source_idx, target_idx)
            else:
                dt_value = -100
            
            is_labeled = 1 if (source_idx < self.data_labeled.shape[0] and target_idx < self.data_labeled.shape[0]) else 0
            is_labeled = torch.tensor(is_labeled, dtype=torch.float)
            ...
            # TODO: check whether returning None for source_idx and target_idx leads to any bugs in rest of the codebase.
            return {
