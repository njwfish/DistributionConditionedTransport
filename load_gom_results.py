...
def load_gom_data_and_forecast(task_name="GoM", seed=42):
    """
    Load training data and forecast results for the GoM task.
    
    Args:
        task_name (str): Name of the task (default: "GoM")
    ...
    training_data = np.load(f"data/realdata/{task_name}_data.npz")
...
def plot_results(results, output_folder="figures"):
    ...
    Args:
        results (dict): Results dictionary from load_gom_data_and_forecast
    ...
    fig.suptitle(f"GoM Results (Seed {results['metadata']['seed']})")
    ...
    for i, X in enumerate(training['Xs']):
        ...
        ax.scatter(X[:, 0], X[:, 1], alpha=0.7, s=0.8, color=color)
...
def plot_trajectories(results, output_folder="figures"):
    ...
    fig, ax = plt.subplots(1, 1, figsize=(10, 8))
    fig.suptitle(f"GoM Trajectories (Seed {results['metadata']['seed']})")
...
def calculate_mmd_scores(task_name="GoM", seeds=[1, 2, 3, 4, 5, 40, 41, 42, 43, 44]):
    """
...
def calculate_emd_scores(task_name="GoM", seeds=[1, 2, 3, 4, 5, 40, 41, 42, 43, 44]):
    """
...
if __name__ == "__main__":
    ...
    results = load_gom_data_and_forecast(task_name="GoM", seed=42)
    ...
    print("\n" + "="*60)
    mmd_results = calculate_mmd_scores(task_name="GoM", seeds=[1, 2, 3, 4, 5, 40, 41, 42, 43, 44])
    
    # Calculate EMD scores across all seeds
    print("\n" + "="*60)
    emd_results = calculate_emd_scores(task_name="GoM", seeds=[1, 2, 3, 4, 5, 40, 41, 42, 43, 44])
