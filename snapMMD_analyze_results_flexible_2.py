...
def load_models_from_experiment(experiment_dir: str, device: torch.device):
    ...
    generator_source_only = bool(experiment_cfg.get('generator_source_only', False))
    ...
    return cfg, enc, gen, generator_source_only


def generate_cde_forecast(experiment_dir: str, training_data: Dict[str, Any]):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    cfg, enc, gen, generator_source_only = load_models_from_experiment(experiment_dir, device)
    ...
    with torch.no_grad():
        enc_s = enc(samples_s)
        # Determine enc_t: if generator is source-only, we set None; otherwise encode target directly
        if generator_source_only:
            enc_t = None
        else:
            enc_t = enc(samples_t)
...
def main():
    ...
    # Predictor is not used in this analysis; removing predictor-related options
