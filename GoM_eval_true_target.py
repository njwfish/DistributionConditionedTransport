...
def run_analysis(predictor_loss_weight, selective_pairing_mode, predictor_type = None, two_step = False):
    ...
    for j, ckpt_dir in enumerate(matching_dirs):
        ...
        if j == plot_seed:
            ...
            plt.savefig(f"outputs/snapMMD_TDE_GoM_true_target_latent_{predictor_type}_{predictor_loss_weight}_{selective_pairing_mode}.png")
...
run_analysis(0.1, "single_step", predictor_type = "cotrained", two_step = False)
run_analysis(0.1, None, predictor_type = "cotrained", two_step = False)
run_analysis(0.1, "single_step", predictor_type = "posthoc", two_step = True)
run_analysis(0.1, None, predictor_type = "posthoc", two_step = True)
