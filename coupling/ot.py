...
class OTCollate(nn.Module):
    ...
    def __call__(self, batch, ot_type: Optional[str] = None, ot_params: Optional[dict] = None):
        ...
        # Apply OT coupling to each individual sample BEFORE batching
        processed_samples = []
        
        # Process each sample individually
        for i, sample in enumerate(batch):
            x_source = sample['source_samples']  # Shape: (set_size, dim)
            x_target = sample['target_samples']  # Shape: (set_size, dim)
            ...
            cost = self.pairwise_dist_fn(x_source_flat, x_target_flat)
