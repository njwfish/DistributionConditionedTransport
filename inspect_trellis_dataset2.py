...
def main():
    ...
    dataset = trellis_dataset(
        split_name='pdo27',  # default split
        set_size=32,
        seed=0
    )
    ...
    # Get all unique patient labels
    # Note: each sample is a tuple: (culture, x0, _, cell_cond, treat_cond, patient)
    unique_patients = []
    for sample in dataset.samples:
        patient_label = sample[5]  # patient is at index 5
        if patient_label not in unique_patients:
            unique_patients.append(patient_label)
