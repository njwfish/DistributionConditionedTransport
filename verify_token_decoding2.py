...
def check_group(entry: dict, esm_tok: EsmTokenizer, progen_tok: AutoTokenizer, num_samples: int, rng: random.Random) -> List[Tuple[int, bool, bool, int, str, str, str]]:
    """
    For a given time-group entry, check a subset of indices and return per-index match results.

    Returns list of tuples: (index, esm_match, progen_match, seq_len, raw_text, decoded_esm, decoded_progen)
    """
    ...
    for idx in indices:
        ...
        if esm_ids is not None:
            ...
            # ESM decoded should be compared with raw sequence minus 2 amino acids
            esm_match = (decoded_esm == raw[:-2])

        if progen_ids is not None:
            decoded_progen = decode_progen(progen_ids[idx].cpu(), progen_tok)
            # ProGen decoded should be compared with raw sequence minus 1 amino acid
            progen_match = (decoded_progen == raw[:-1])
        ...
        results.append((idx, bool(esm_match) if esm_match is not None else False,
                        ...
                        len(raw), raw, decoded_esm, decoded_progen))
...
def main():
    ...
    for entry in data:
        if groups_checked >= args.timepoints:
            break
        time_label = entry.get('time-loc', 'UNKNOWN')
        ...
        for (idx, esm_match, progen_match, seq_len, raw_text, decoded_esm, decoded_progen) in results:
            ...
            # Print sequences for visual comparison when there's a mismatch
            if ('esm_input_ids' in samples and not esm_match) or ('progen_input_ids' in samples and not progen_match):
                print(f"    Original: {raw_text}")
                if 'esm_input_ids' in samples and not esm_match:
                    print(f"    ESM target (raw[:-2]): {raw_text[:-2]}")
                    print(f"    ESM decoded: {decoded_esm}")
                if 'progen_input_ids' in samples and not progen_match:
                    print(f"    ProGen target (raw[:-1]): {raw_text[:-1]}")
