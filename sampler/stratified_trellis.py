...
class StratifiedBatchSamplerTrellis(BaseSampler):
    ...
    def _categorize_indices(self):
        ...
        # Regular trellis style dataset
        elif hasattr(self.dataset, 'num_samples'):
            n = self.dataset.num_samples
            
            # Consecutive pairs are on the diagonal: source_idx == target_idx
            for i in range(n):
                idx = i * n + i
                self.consecutive_indices.append(idx)
            ...
            raise ValueError(
                "Dataset must have either (n_train, total_x_populations) attributes "
                "for trellis_a2a style, or num_samples attribute for regular trellis style. "
                "Use the standard StratifiedBatchSampler for other dataset types."
            )
...
class StratifiedWeightedSamplerTrellis(BaseSampler):
    ...
    def _compute_weights(self) -> torch.Tensor:
        ...
        # Regular trellis style dataset
            ...
            # Consecutive pairs are on the diagonal: source_idx == target_idx
            # idx = source_idx * n + target_idx, consecutive when source_idx == target_idx
            for i in range(n):
                idx = i * n + i  # diagonal
                weights[idx] = self.consecutive_weight
            
            print(f"[StratifiedWeightedSamplerTrellis] Computed weights analytically for {dataset_len} items "
                  f"({n} consecutive pairs with weight {self.consecutive_weight})")
        
        else:
            raise ValueError(
                "Dataset must have either (n_train, total_x_populations) attributes "
                "for trellis_a2a style, or num_samples attribute for regular trellis style. "
                "Use the standard StratifiedWeightedSampler for other dataset types."
            )
