...
def main():
    parser = argparse.ArgumentParser(description="Filter sequences with unusual tokens for ESM2 and ProGen2, keeping only first 10 valid sequences per element")
    ...
    parser.add_argument("--data_out", default="data/spikeprot0430/virus_tokenized_data_for_tde_downsampled100_filtered_extreme.pt", help="Path to save filtered dataset .pt (default: <data_in>.filtered.pt)")
    ...
    for el in data:
        ...
        # Get indices that passed the original filtering
        passing_indices = keep_mask.nonzero(as_tuple=False).view(-1).tolist()
        
        # Limit to first 10 sequences that passed the filter
        limited_indices = passing_indices[:10]
        
        # Create a new mask for only the first 10 passing sequences
        limited_keep_mask = torch.zeros_like(keep_mask)
        if limited_indices:
            limited_keep_mask[limited_indices] = True
        
        # Apply the limited mask across all per-sequence arrays inside samples
        apply_keep_mask_to_samples(samples, limited_keep_mask)
        
        # Also apply mask to raw_texts
        el["raw_texts"] = [el["raw_texts"][i] for i in limited_indices]
