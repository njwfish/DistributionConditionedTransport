...
class DTConditionedPredictor(nn.Module):
    ...
    @staticmethod
    def create_mlp(latent_dim, hidden_dim=128, num_layers=2):
        return DTConditionedPredictor(
    ...
    @staticmethod
    def create_ridge(latent_dim, ridge_alpha=1e-3):
        return DTConditionedPredictor(
            latent_dim=latent_dim,
            model_type="ridge",
            conditioning_mode="none",
    ...
    def forward(self, x, d=None):
        if self.conditioning_mode == "none":
            ...
            if d is None:
                raise ValueError("d must be provided when conditioning_mode is not 'none'")
            d = d.float()
    ...
    def loss(self, source_latent, target_latent, d=None):
