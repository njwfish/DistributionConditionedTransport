...
class LTSeqDatasetUnstructured(Dataset):
    ...
    def __getitem__(self, idx):
        if self.mode == 'train':
            # independent coupling rate
            use_guess = random.random() < 0.75
...
class LTSeqDatasetUnstructuredBidirectional(Dataset):
    """Dataset for lineage-traced scRNA-seq from Weinreb, et al., 2020
    bidirectional pairing policy
    """
    ...
    def generate_clone_set_pairs(self, test_split: float = 0.9):
        ...
        # now, let's split these clones into train and test sets!
        clones = sorted(list(pairable_clones)) # sort for reproducibility!
        np.random.seed(self.seed)
        np.random.shuffle(clones)
    ...
    def clone_set_random_pairing(self):
        """bidirectional: any pair of times"""
        ...
        lonely_clones = set()
        for c, t in clone_map:
            if not any((c, ot) in clone_map for ot in all_times if ot != t):
                lonely_clones.add(c)

        srcs, tgts, src_meta, tgt_meta = [], [], [], []
        for c in lonely_clones:
            for t in all_times:
                if (c, t) not in clone_map:
                    continue
                for tgt_t in all_times:
                    if tgt_t == t:
                        continue
                    if time_map[tgt_t]:
                        for src_i in clone_map[(c, t)]:
