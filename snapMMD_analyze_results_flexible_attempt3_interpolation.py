...
DATASET_CONFIGS: Dict[str, Dict[str, Any]] = {
    'LV': {
        ...
        'data_path_interpolation': 'data/classic/LV_data_interp_val.npz',        
    ...
    'Repressilator': {
        ...
        'data_path_interpolation': 'data/classic/Repressilator_data_interp_val.npz',
        'dimensionality': 3,
    ...
    'GoM': {
        ...
        'data_path_interpolation': 'data/realdata/GoM_data_interp_val.npz',
        'dimensionality': 2,
        'axes_labels': ['X1', 'X2'],
        'title': 'GoM',
        'calculate_emd': True,
    ...
    'pbmc': {
        ...
        'data_path_interpolation': 'data/realdata/processed_pbmc_data_sub500_every_2_until20_interp_val.npz',
        'dimensionality': 30,
...
def generate_cde_interpolations(
    experiment_dir: str,
    interpolation_data: Dict[str, Any],
    latent_mode: str = 'adjacent',  # 'adjacent' or 'true'
    cli_set_size: Optional[int] = None,
    predictor_source: str = 'separate',
    predictor_dir: Optional[str] = None,
):
    """Generate predictions for all odd (interpolated) timepoints.
    latent_mode:
      - 'adjacent': enc_t = 0.5 * (enc(even_left) + enc(even_right))
      - 'true':     enc_t = enc(odd)
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # Load models (predictor is unused here unless extended later)
    cfg, enc, gen, predictor, dataset, _ = load_models_from_experiment(
        experiment_dir,
        device,
        predictor_source=predictor_source,
        predictor_dir=predictor_dir,
        use_true_target_latent=(latent_mode == 'true'),
    )

    # Resolve set_size from config with optional CLI override
    cfg_resolved = OmegaConf.to_container(cfg, resolve=True)
    set_size_cfg = None
    if isinstance(cfg_resolved, dict):
        exp_cfg = cfg_resolved.get('experiment', {})
    ...
    if set_size_cfg is None:
        set_size_cfg = int(getattr(dataset, 'set_size', 32))
    if cli_set_size is not None:
        set_size_cfg = int(cli_set_size)

    Xs_even: List[np.ndarray] = interpolation_data['Xs_even']
    Xs_odd: List[np.ndarray] = interpolation_data['Xs_odd']

    forecasts_by_odd: List[np.ndarray] = []

    for j in range(len(Xs_odd)):
        X_left = Xs_even[j]
        X_right = Xs_even[j + 1]
        X_target_odd = Xs_odd[j]

        # Build subsets from the left (source) set to cover every point once
        num_available = int(X_left.shape[0])
        perm_indices = np.random.permutation(num_available)
        num_full_sets = num_available // set_size_cfg
        remainder = num_available % set_size_cfg
        ...
        per_odd_agg: List[np.ndarray] = []

        # Process full subsets
        for subset_indices in full_subsets:
            # Source (left even)
            src_subset_np_left_lat = X_left[subset_indices]
            src_subset_t_left_lat = torch.tensor(src_subset_np_left_lat, dtype=torch.float32, device=device).unsqueeze(0)

            with torch.no_grad():
                enc_left = enc(src_subset_t_left_lat)

                if latent_mode == 'true':
                    tgt_subset_np_odd = X_target_odd[subset_indices]
                    tgt_subset_t_odd = torch.tensor(tgt_subset_np_odd, dtype=torch.float32, device=device).unsqueeze(0)
                    enc_t = enc(tgt_subset_t_odd)
                elif latent_mode == 'adjacent':
                    # Right even latents (matching indices)
                    src_subset_np_right_lat = X_right[subset_indices]
                    src_subset_t_right_lat = torch.tensor(src_subset_np_right_lat, dtype=torch.float32, device=device).unsqueeze(0)
                    enc_right = enc(src_subset_t_right_lat)
                    enc_t = 0.5 * (enc_left + enc_right)
                else:
                    raise ValueError(f"Unsupported latent_mode for interpolation: {latent_mode}")

                # Generator input uses the source left set
                src_subset_t_gen = src_subset_t_left_lat
                _, set_size_cur, *data_shape = src_subset_t_gen.shape
                gen_src = src_subset_t_gen.reshape(-1, *data_shape)

                pred_subset = gen.sample(gen_src, enc_left, enc_t)[0]
                per_odd_agg.append(pred_subset.detach().cpu().numpy())

        # Process leftover subset (if any): latents padded to set_size, generator sees only true leftover indices
        if leftover_indices is not None and padded_latent_indices is not None:
            src_subset_np_left_lat = X_left[padded_latent_indices]
            src_subset_t_left_lat = torch.tensor(src_subset_np_left_lat, dtype=torch.float32, device=device).unsqueeze(0)

            with torch.no_grad():
                enc_left = enc(src_subset_t_left_lat)
                if latent_mode == 'true':
                    tgt_subset_np_odd = X_target_odd[padded_latent_indices]
                    tgt_subset_t_odd = torch.tensor(tgt_subset_np_odd, dtype=torch.float32, device=device).unsqueeze(0)
                    enc_t = enc(tgt_subset_t_odd)
                elif latent_mode == 'adjacent':
                    src_subset_np_right_lat = X_right[padded_latent_indices]
                    src_subset_t_right_lat = torch.tensor(src_subset_np_right_lat, dtype=torch.float32, device=device).unsqueeze(0)
                    enc_right = enc(src_subset_t_right_lat)
                    enc_t = 0.5 * (enc_left + enc_right)

                # Generator uses only leftover true indices on left
                src_subset_np_left_gen = X_left[leftover_indices]
                src_subset_t_gen = torch.tensor(src_subset_np_left_gen, dtype=torch.float32, device=device).unsqueeze(0)
                _, set_size_cur, *data_shape = src_subset_t_gen.shape
                gen_src = src_subset_t_gen.reshape(-1, *data_shape)
        ...
        forecasts_by_odd.append(np.vstack(per_odd_agg))

    return {
        'forecasts_by_odd': forecasts_by_odd,
        'Xs_odd_true': Xs_odd,
    }
...
def compute_mmd_and_emd(dataset_name: str, forecast_NxD: np.ndarray, logger: logging.Logger, enable_emd: bool = True) -> Tuple[float, float, Optional[float]]:
...
def compute_mmd_and_emd_between(dataset_name: str, pred_NxD: np.ndarray, true_NxD: np.ndarray, logger: logging.Logger, enable_emd: bool = True, log_details: bool = True) -> Tuple[float, float, Optional[float]]:
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    pred_t = torch.from_numpy(pred_NxD).to(device)
    true_t = torch.tensor(true_NxD).to(device)

    mmd_legacy = np.sqrt(mmd_loss_legacy(pred_t, true_t).item())
    mmd_paper = mmd_loss_paper(pred_t, true_t).item()

    emd = None
    if enable_emd and DATASET_CONFIGS[dataset_name].get('calculate_emd', False):
        emd_val = calculate_emd(pred_NxD, true_NxD)
        emd = float(emd_val) if not np.isnan(emd_val) else None

    if log_details:
        logger.info(f"Computed metrics (between) -> MMD_legacy: {mmd_legacy:.6f}, MMD_paper: {mmd_paper:.6f}, EMD: {('%.6f' % emd) if emd is not None else 'n/a'}")
    return mmd_legacy, mmd_paper, emd
...
def plot_interpolation_results(dataset_name: str, interpolation_data: Dict[str, Any], forecasts_by_odd: Optional[List[np.ndarray]], out_dir: str, logger: logging.Logger, pca: PCA = None, title_suffix: str = ""):
    os.makedirs(out_dir, exist_ok=True)
    cfg = DATASET_CONFIGS[dataset_name]

    combined_seq: List[np.ndarray] = interpolation_data['combined_sequence']
    is_3d = cfg.get('plot_dimensionality', cfg['dimensionality']) == 3
    if is_3d:
        fig = plt.figure(figsize=(12, 8))
        ax = fig.add_subplot(111, projection='3d')
    ...
    fig.suptitle(f"{cfg['title']} Interpolation Results (CDE){(' | ' + title_suffix) if title_suffix else ''}")

    # Plot all timepoints (even and odd) in correct order
    n_sequences = len(combined_seq)
    all_points = []
    colors = []
    for i, X in enumerate(combined_seq):
        Xp = transform_for_plot(dataset_name, X, pca)
        all_points.append(Xp)
        colors.extend([i + 1] * len(Xp))
    all_points = np.concatenate(all_points, axis=0)

    if is_3d:
        scatter = ax.scatter(all_points[:, 0], all_points[:, 1], all_points[:, 2], alpha=0.7, s=3.0, c=colors, cmap='coolwarm', vmin=1, vmax=n_sequences)
    else:
        scatter = ax.scatter(all_points[:, 0], all_points[:, 1], alpha=0.7, s=3.0, c=colors, cmap='coolwarm', vmin=1, vmax=n_sequences)
    cbar = plt.colorbar(scatter, ax=ax, shrink=0.8, aspect=20)
    cbar.set_label('Time Point (even+odd)', rotation=270, labelpad=15)

    # Overlay interpolated predictions vs odd-time ground truths
    if forecasts_by_odd is not None:
        Xs_odd_truth: List[np.ndarray] = interpolation_data['Xs_odd']
        first_overlay = True
        for j, pred_np in enumerate(forecasts_by_odd):
            true_np = Xs_odd_truth[j]
            true_p = transform_for_plot(dataset_name, true_np, pca)
            pred_p = transform_for_plot(dataset_name, pred_np, pca)

            gt_label = 'True (odd times)' if first_overlay else None
            fc_label = 'Interpolated forecast (odd times)' if first_overlay else None
    ...
    if is_3d:
        ax.set_zlabel(cfg['axes_labels'][2] if len(cfg['axes_labels']) > 2 else 'Z')
    ax.set_title('All Data (even+odd) with Interpolated Predictions')
    ax.legend()
    plt.tight_layout()

    out_path = os.path.join(out_dir, f"{dataset_name}_results_CDE_interpolation.png")
    plt.savefig(out_path, dpi=300, bbox_inches='tight')
    plt.close()
    logger.info(f"Interpolation figure saved to: {out_path}")
...
def load_interpolation_data_for_dataset(dataset_name: str) -> Dict[str, Any]:
    if dataset_name not in DATASET_CONFIGS:
        raise ValueError(f"Dataset {dataset_name} is not supported. Supported: {list(DATASET_CONFIGS.keys())}")
    cfg = DATASET_CONFIGS[dataset_name]
    data_even = np.load(cfg['data_path'])
    data_odd = np.load(cfg['data_path_interpolation'])

    Xs_even_full = [data_even['Xs'][i] for i in range(data_even['N_steps'])]
    Xs_odd_full = [data_odd['Xs'][i] for i in range(data_odd['N_steps'])]

    # Interleave even and odd to get the full ordered sequence
    combined_sequence: List[np.ndarray] = []
    for i in range(len(Xs_odd_full)):
        combined_sequence.append(Xs_even_full[i])
        combined_sequence.append(Xs_odd_full[i])
    combined_sequence.append(Xs_even_full[-1])

    return {
        'Xs_even': Xs_even_full,           # length Neven
        'Xs_odd': Xs_odd_full,             # length Nodd == Neven-1
        'combined_sequence': combined_sequence,
        'N_steps_even': data_even['N_steps'],
        'N_steps_odd': data_odd['N_steps'],
        'dts_even': data_even['dts'],
        'dts_odd': data_odd['dts'],
        'y0': data_even['y0'],
        'time_scale': data_even['time_scale'],
    }
...
def main():
    ...
    parser.add_argument('--predictor-source', type=str, default='separate', help='Directory containing predictor subdirectories')
    ...
    # Interpolation branch options
    parser.add_argument('--interpolation', action='store_true',
                        help='Run interpolation evaluation instead of forecasting')
    parser.add_argument('--interpolation-latent-mode', type=str, default='adjacent', choices=['adjacent', 'true'],
                        help='How to obtain target latent for interpolation: adjacent-averaged or true (encoded odd)')
    ...
    if args.interpolation:
        interpolation_data = load_interpolation_data_for_dataset(dataset_name)
    ...
    interpolation_results_per_seed: List[Tuple[int, List[np.ndarray], List[Tuple[float, float, Optional[float]]]]] = []
    ...
    for seed, (exp_seed, (exp_dir, cfg)) in zip(seeds_sorted, matched_with_seed):
        ...
        if not args.interpolation:
            ...
            interp_out = generate_cde_interpolations(
                exp_dir,
                interpolation_data,
                latent_mode=args.interpolation_latent_mode,
                cli_set_size=args.set_size,
                predictor_source=args.predictor_source,
                predictor_dir=predictor_dir,
            )
            forecasts_by_odd = interp_out['forecasts_by_odd']
            Xs_odd_true = interp_out['Xs_odd_true']

            metrics_for_seed: List[Tuple[float, float, Optional[float]]] = []
            for j in range(len(forecasts_by_odd)):
                pred_j = forecasts_by_odd[j]
                true_j = Xs_odd_true[j]
                mmd_legacy_j, mmd_paper_j, emd_j = compute_mmd_and_emd_between(
                    dataset_name,
                    pred_j,
                    true_j,
                    logger,
                    enable_emd=not args.disable_emd,
                    log_details=False,
                )
                metrics_for_seed.append((mmd_legacy_j, mmd_paper_j, emd_j))

            interpolation_results_per_seed.append((seed, forecasts_by_odd, metrics_for_seed))
    ...
    if not args.skip_plots:
        if forecast_for_plot is None:
            ...
            if (not args.use_true_target_latent) and (not args.interpolation):
                try:
        ...
        if not args.interpolation:
            ...
            # Interpolation plotting and aggregate metrics
            if len(interpolation_results_per_seed) == 0:
                logger.warning("No interpolation results available; skipping plots.")
            else:
                # Aggregate metrics across odd timepoints and seeds
                mmd_paper_vals = []
                mmd_legacy_vals = []
                emd_vals = []
                for (seed, _, metrics_for_seed) in interpolation_results_per_seed:
                    for (mmd_legacy_j, mmd_paper_j, emd_j) in metrics_for_seed:
                        mmd_paper_vals.append(mmd_paper_j)
                        mmd_legacy_vals.append(mmd_legacy_j)
                        if emd_j is not None:
                            emd_vals.append(emd_j)

                mmd_paper_arr = np.array(mmd_paper_vals)
                mmd_legacy_arr = np.array(mmd_legacy_vals)
                mmd_paper_mean = float(mmd_paper_arr.mean()) if len(mmd_paper_arr) > 0 else float('nan')
                mmd_legacy_mean = float(mmd_legacy_arr.mean()) if len(mmd_legacy_arr) > 0 else float('nan')
                mmd_paper_std = float(mmd_paper_arr.std()) if len(mmd_paper_arr) > 0 else float('nan')
                mmd_legacy_std = float(mmd_legacy_arr.std()) if len(mmd_legacy_arr) > 0 else float('nan')
                emd_title_segment = ""
                if len(emd_vals) > 0:
                    emd_arr = np.array(emd_vals)
                    emd_mean = float(emd_arr.mean())
                    emd_std = float(emd_arr.std())
                    logger.info(f"Interpolation EMD: {emd_mean:.6f} ± {emd_std:.6f}")
                    emd_title_segment = f" | EMD={emd_mean:.4g}±{emd_std:.2g}"

                # Per-index (odd index) aggregation across seeds
                # Determine number of odd indices from first seed entry
                num_odd = len(interpolation_results_per_seed[0][2])
                logger.info("Interpolation results aggregated by index (across seeds):")
                for j in range(num_odd):
                    mmd_paper_j_vals = []
                    mmd_legacy_j_vals = []
                    emd_j_vals = []
                    for (_, _, metrics_for_seed) in interpolation_results_per_seed:
                        mmd_legacy_j, mmd_paper_j, emd_j = metrics_for_seed[j]
                        mmd_paper_j_vals.append(mmd_paper_j)
                        mmd_legacy_j_vals.append(mmd_legacy_j)
                        if emd_j is not None:
                            emd_j_vals.append(emd_j)
                    mmd_paper_j_mean = float(np.mean(mmd_paper_j_vals)) if len(mmd_paper_j_vals) > 0 else float('nan')
                    mmd_paper_j_std = float(np.std(mmd_paper_j_vals)) if len(mmd_paper_j_vals) > 0 else float('nan')
                    mmd_legacy_j_mean = float(np.mean(mmd_legacy_j_vals)) if len(mmd_legacy_j_vals) > 0 else float('nan')
                    mmd_legacy_j_std = float(np.std(mmd_legacy_j_vals)) if len(mmd_legacy_j_vals) > 0 else float('nan')
                    if len(emd_j_vals) > 0:
                        emd_j_mean = float(np.mean(emd_j_vals))
                        emd_j_std = float(np.std(emd_j_vals))
                        emd_part = f", EMD = {emd_j_mean:.6f} ± {emd_j_std:.6f}"
                    else:
                        emd_part = ""
                    full_index = 2 * j + 1
                    logger.info(
                        f"  Intermediate index {full_index}: MMD_paper = {mmd_paper_j_mean:.6f} ± {mmd_paper_j_std:.6f}, "
                        f"MMD_legacy = {mmd_legacy_j_mean:.6f} ± {mmd_legacy_j_std:.6f}{emd_part}"
                    )

                metrics_title_segment = f"MMD={mmd_paper_mean:.4g}±{mmd_paper_std:.2g}, MMD_legacy={mmd_legacy_mean:.4g}±{mmd_legacy_std:.2g}{emd_title_segment}"
                title_suffix = f"{parameters_label_for_title} | {metrics_title_segment}"

                # Plot combined data once (no per-seed difference in the data) and overlay predictions from default seed if present, else first seed
                default_seed_entry = None
                for e in interpolation_results_per_seed:
                    if e[0] == default_seed_for_plots:
                        default_seed_entry = e
                        break
                if default_seed_entry is None:
                    default_seed_entry = interpolation_results_per_seed[0]
                forecasts_by_odd_default = default_seed_entry[1]
                plot_interpolation_results(dataset_name, interpolation_data, forecasts_by_odd_default, out_dir, logger, pca, title_suffix=title_suffix)
