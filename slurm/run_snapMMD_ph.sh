...
#SBATCH --mem 5GB
#SBATCH -o logs/oph_%a
#SBATCH -e logs/eph_%a
#SBATCH --array=0-4:1
...
dataset_idx=$((SLURM_ARRAY_TASK_ID / (num_selective_pairing_modes * num_seeds * num_alpha)))
...
echo "Running job ${SLURM_ARRAY_TASK_ID}: dataset=${dataset_name}, selective_pairing_mode=${selective_pairing_mode}, seed=${seed}"
...
python main.py experiment=snapMMD_energy_cotrain dataset_name=${dataset_name} experiment.predictor_loss_weight=${alpha} experiment.selective_pairing_mode=${selective_pairing_mode} seed=${seed} experiment.ot_coupling=${ot_coupling} training.num_epochs=500 predictor_experiment_name=default