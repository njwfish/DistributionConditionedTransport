#!/bin/bash
#SBATCH --chdir=/orcd/archive/abugoot/001/Projects/paolo/main_tde
#SBATCH -t 00:30:00
#SBATCH --gres shard:1
#SBATCH --constraint any-A100
#SBATCH --partition abugoot
#SBATCH --mem 2GB
#SBATCH -o logs/om_%a
#SBATCH -e logs/em_%a
#SBATCH --array=0-4:1

export HYDRA_FULL_ERROR=1
export WANDB_API_KEY="c72e34cd8dc67f7220e3517232e86861cd5c537b"

# Define dataset names (only GoM and PBMC)
datasets=("GoM")

selective_pairing_modes=("single_step")

# Define seeds
seeds=(40 41 42 43 44)


# Calculate indices for this array task
# Total combinations: 4 datasets × 2 weight_modes × 3 selective_pairing_modes × 10 seeds = 240 jobs
num_datasets=${#datasets[@]}                    # 4
num_selective_pairing_modes=${#selective_pairing_modes[@]} # 3
num_seeds=${#seeds[@]}                          # 10

# Calculate which combination this job should run
dataset_idx=$((SLURM_ARRAY_TASK_ID / (num_selective_pairing_modes * num_seeds)))
selective_pairing_mode_idx=$(((SLURM_ARRAY_TASK_ID % (num_selective_pairing_modes * num_seeds)) / (num_seeds)))
seed_idx=$(((SLURM_ARRAY_TASK_ID % (num_seeds))))

# Get the actual values for this combination
dataset_name=${datasets[$dataset_idx]}
selective_pairing_mode=${selective_pairing_modes[$selective_pairing_mode_idx]}
seed=${seeds[$seed_idx]}

echo "Running job ${SLURM_ARRAY_TASK_ID}: dataset=${dataset_name}, selective_pairing_mode=${selective_pairing_mode}, seed=${seed}"

# Set ot_coupling based on dataset: true for PBMC, false for others
if [ "$dataset_name" == "PBMC" ]; then
    ot_coupling=true
else
    ot_coupling=false
fi

# Run the unified hyperparameter experiment with the specified hyperparameters
python main.py experiment=snapMMD_mfm dataset_name=${dataset_name} experiment.selective_pairing_mode=${selective_pairing_mode} seed=${seed} experiment.ot_coupling=${ot_coupling} +model.source_only=true wandb=offline